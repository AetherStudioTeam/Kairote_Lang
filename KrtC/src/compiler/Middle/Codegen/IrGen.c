...
static void KrtIrEnsureMainEntry(KrtIRBuilder* builder) {
    ...
    if (!builder->module->main_function) {
        ...
        KrtIRFunction* main_func = KrtIrFunctionCreate(builder, "_KrtMainEntry", NULL, 0, TOKEN_VOID);
...
static KrtIRValue KrtIrGenerateExpression(KrtIRBuilder* builder, ASTNode* expr) {
    ...
    switch (expr->type) {
        ...
        case AST_BINARY_OPERATION: {
            ...
            switch (expr->data.binary_op.operator) {
                ...
                case TOKEN_AND: {
                    ...
                    char rhs_label[IRGEN_LABEL_BUFFER_SIZE], end_label[IRGEN_LABEL_BUFFER_SIZE], result_var[32];
                    ...
                    KrtIrStore(builder, result_var, KRT_IMM_ZERO(builder));
                    ...
                    KrtIrStore(builder, result_var, KRT_IMM_ONE(builder));
        ...
        case AST_CALL: {
            ...
            if (expr->data.call.argument_count > 0) {
                KrtTokenType* param_types = (KrtTokenType*)KRT_MALLOC(expr->data.call.argument_count * sizeof(KrtTokenType));
                if (param_types) {
                    for (int i = 0; i < expr->data.call.argument_count; i++) {
                        param_types[i] = TOKEN_IDENTIFIER;
                    }
                    call_mangled = name_mangle_function(NULL, func_name, param_types, expr->data.call.argument_count);
        ...
        case AST_TERNARY_OPERATION: {
            ...
            snprintf(true_label, sizeof(true_label), "if_true_%d", builder->label_counter++);
            snprintf(false_label, sizeof(false_label), "if_false_%d", builder->label_counter++);
            snprintf(end_label, sizeof(end_label), "if_end_%d", builder->label_counter++);
        ...
        case AST_VARIABLE_DECLARATION: {
            ...
            if (is_c_style_array) {
                ...
                if (stmt->data.variable_decl.value)
                    KrtIrStore(builder, stmt->data.variable_decl.name,
                                KrtIrGenerateExpression(builder, stmt->data.variable_decl.value));
            }
            break;
        }
                ...
                if (stmt->data.namespace_decl.body) {
                    if (stmt->data.namespace_decl.body->type == AST_BLOCK) {
                        for (int i = 0; i < stmt->data.namespace_decl.body->data.block.statement_count; i++)
                            KrtIrGenerateStatement(builder, stmt->data.namespace_decl.body->data.block.statements[i]);
                    } else {
...
void KrtIrGenerateFromAst(KrtIRBuilder* builder, ASTNode* ast, TypeCheckContext* type_context) {
    ...
    if (ast->type == AST_BLOCK || ast->type == AST_PROGRAM) {
        ...
        for (int i = 0; i < ast->data.block.statement_count; i++)
            if (ast->data.block.statements[i]->type == AST_NAMESPACE_DECLARATION)
                KrtIrGenerateStatement(builder, ast->data.block.statements[i]);

        // 然后处理函数和类声明
        for (int i = 0; i < ast->data.block.statement_count; i++)
            if (ast->data.block.statements[i]->type == AST_FUNCTION_DECLARATION ||
                ast->data.block.statements[i]->type == AST_STATIC_FUNCTION_DECLARATION ||
                ast->data.block.statements[i]->type == AST_CLASS_DECLARATION)
                KrtIrGenerateStatement(builder, ast->data.block.statements[i]);
