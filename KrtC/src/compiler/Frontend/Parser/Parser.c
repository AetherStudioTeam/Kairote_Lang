...
ASTNode* parser_parse(Parser* parser) {
    ...
    printf("[DEBUG PARSER] Starting parse, first token type=%d, value=%s\n", 
    ...
    printf("[DEBUG PARSER] Parse complete, %d statements\n", statement_count);
    for (int i = 0; i < statement_count && i < 5; i++) {
        printf("[DEBUG PARSER] Statement[%d]: type=%d\n", i, statements[i] ? statements[i]->type : -1);
    }
