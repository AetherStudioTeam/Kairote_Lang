...
#define PLATFORM_H
    ...
    #define KrtWriteFile(handle, buffer, count, written, overlapped) \
        do { DWORD _written; WriteFile(handle, buffer, count, &_written, overlapped); *(written) = _written; } while(0)
    #define KrtReadFile(handle, buffer, count, read, overlapped) \
    ...
    #define KrtCreateDirectory(path) CreateDirectory(path, NULL)
    ...
    #define KrtCreateDirectory(path) mkdir(path, 0755)
